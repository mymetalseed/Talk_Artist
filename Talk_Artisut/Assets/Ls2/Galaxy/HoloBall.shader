Shader "Unlit/HoloBall"
{
    Properties
    {
        ...
        _Color("_Color",Color) = (0.5,0.3,0.2,0.5)
        ...
        _Center("Center",Vector) = (0,1,1,1)

        _MaxScanDistance("MaxScanDistance",Float) = 1
        _ScanLineWidth("ScanLineWidth",Float) = 1
    }
    SubShader
    {
        Tags { "RenderType"="Transparent" "LightMode"="ForwardBase" "Queue" = "Transparent" "IgnoreProjection" = "True"}
        Zwrite On
        ...
        Pass
        {
            ...
            vertex2FragmentData vert (MeshData v)
            {
                ...
                float distLv = saturate(saturate(v.vertex.z-_Center.z*_CosTime.y) / _MaxScanDistance);

                float inner = step(distLv, 0.1);
                float outer = step(distLv, _ScanLineWidth);


                v.vertex = v.vertex * saturate(1-outer+inner);
            ...
            fixed4 frag (vertex2FragmentData i) : SV_Target
            {
                ...
                return fixed4(FresnelColor,_Color.a);
            }
            ENDCG
        }
    }
